...
@click.option("--num-parts", default=2, type=int, help="Number of parts to split the subdirectories into (default: 2)")
def main(source_dir, target_dir, num_parts):
    # Create directories for parts if they don't exist
    part_dirs = [target_dir[0] / f"part-{i}" for i in range(1, num_parts + 1)]
    ...
    subdirs_per_part = total_subdirs // num_parts

    # Split the list of subdirectories into parts
    subdirs_parts = [subdirs[i:i + subdirs_per_part] for i in range(0, total_subdirs, subdirs_per_part)]
